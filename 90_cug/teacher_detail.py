...
def extract_teacher_info(html_content):
    ...
    profile_section = soup.find('div', class_='t_grjj_nr')
    if profile_section:
        teacher_info['个人简介'] = profile_section.get_text(strip=True)

    # Extracting education background
    education_section = soup.find('div', class_='bd')
    education_list = []
    if education_section:
        education_items = education_section.find_all('ul', class_='jyjlulu13')
        for item in education_items:
            education = {}
            dates = item.find_previous('div', class_='t_e_date').get_text(strip=True)
            education['时间'] = dates
            education['学校'] = item.find_all('li')[0].get_text(strip=True)
            education['专业'] = item.find_all('li')[1].get_text(strip=True)
            education['学历'] = item.find_all('li')[2].get_text(strip=True)
            education['学位'] = item.find_all('li')[3].get_text(strip=True)
            education_list.append(education)
        teacher_info['教育经历'] = education_list

    # Extracting work experience
    work_experience_section = soup.find('ul', class_='gzjlu14')
    work_experience_list = []
    if work_experience_section:
        work_experience_items = work_experience_section.find_all('li')
        work_dates = work_experience_section.find_previous('div', class_='t_e_date').get_text(strip=True)
        work_experience = {
            '时间': work_dates,
            '单位': work_experience_items[0].get_text(strip=True),
            '职位': work_experience_items[1].get_text(strip=True),
        }
